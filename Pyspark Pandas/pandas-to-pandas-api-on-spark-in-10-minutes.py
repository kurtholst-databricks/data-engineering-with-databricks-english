...
# To avaid problems and warnings from the Pandas API
...
psdf = ps.from_pandas(pdf).to_pandas()
...
ps.sql("SELECT * FROM {psdf} where pig > 30", psdf=psdf)
...
# This code performs an SQL query to join two DataFrames, 'psdf' and 'pdf', on the 'year' column.
# It selects the 'pig' column from 'psdf' and the 'chicken' column from 'pdf', 
# and orders the results by 'pig' and 'chicken' in ascending order.
...
# Set the option context for operations on different frames and default index type
with option_context(
        "compute.ops_on_diff_frames", True,
        "compute.default_index_type", 'distributed'):
    # Create a DataFrame with a range of 10 and perform an addition operation
    df = ps.range(100) + ps.range(10)
    # Display the physical plan of the DataFrame
    df.spark.explain()
